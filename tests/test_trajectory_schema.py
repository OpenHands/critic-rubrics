...
def test_annotate_conversation_rubric_schema_consistency():
    """Test that AnnotateConversationRubric.tool_description generates parameters
    that match (or are a superset of) ANNOTATION_TOOL parameters."""
    ...
    for name, field in AnnotateConversationRubric.model_fields.items():
    ...
    print(f"✅ All {len(expected_params)} expected parameters found and match exactly")
    print(f"📊 Generated tool has {len(generated_params)} total parameters ({len(generated_params) - len(expected_params)} additional)")
