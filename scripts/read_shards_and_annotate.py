    ...
    for shard in shards:
        ...
        with open_shard_fn(shard) as f:
            for line in f:
                ...
                if trace_segment.follow_up_user_message is not None:
                ...
                output_filepath = output_dir / f"conv_{conversation_id}" / f"{segment_id}.json"
                ...
                rich.print((
                    ...
                    f"[bold yellow]Has follow-up user?[/bold yellow] {'Yes' if trace_segment.follow_up_user_message is not None else 'No':<3} | "
